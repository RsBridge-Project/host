...
ssize_t ksys_load_bpf(const char __user* path, unsigned int length)
{
	char *load_prog = "/home/yjq/eXRP/share/load";
	char *cmd_argv[3];
	char *cmd_envp[1];
	char *p;
	ssize_t ret;
	p = kmalloc(length * sizeof(char), GFP_KERNEL);
	ret = copy_from_user(p, path, length); 
	cmd_argv[0] = load_prog;
	cmd_argv[1] = p;
	cmd_argv[2] = NULL;
	cmd_envp[0] = NULL;
	// 用户态程序用于加载ebpf程序
	ret = call_usermodehelper(load_prog, cmd_argv, cmd_envp, UMH_WAIT_PROC);
	return ret;
}

SYSCALL_DEFINE2(load_bpf, const char __user *, path, unsigned int, length)
{
	return ksys_load_bpf(path, length);
}
