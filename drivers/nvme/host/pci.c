				   ...
				   struct io_comp_batch *iob, u16 idx)
{
	...
	if (!req->bio || !req->bio->xrp_enabled) {
		...
		// printk("DEBUG: nvme_handle_cqe xrp_count %d\n", req->bio->xrp_count);
		if(req->bio->xrp_count > 50)
		{	
			printk("DEBUG: nvme_handle_cqe xrp_count %d\n", req->bio->xrp_count);
			if (!nvme_try_complete_req(req, cqe->status, cqe->result) &&
				!blk_mq_add_to_batch(req, iob, nvme_req(req)->status,
		...
		// printk("nvme_handle_cqe: begin key %ld\n", req->bio->xrp_scratch_page.range_begin);
		...
		if (ebpf_context.done) {
			// printk("DEBUG nvme_handle_cqe done\n");
			// printk("xrp_scratch_page value_ptr: %lu\n",req->bio->xrp_scratch_page.value_ptr);
			// printk("xrp_scratch_page state_flags: %u\n",req->bio->xrp_scratch_page.state_flags);
			// printk("xrp_scratch_page key: %lu\n",req->bio->xrp_scratch_page.keys[0]);
			// printk("xrp_scratch_page value: %s\n",req->bio->xrp_scratch_page.values[0].value);
			// printk("xrp_scratch_page found: %d\n",req->bio->xrp_scratch_page.values[0].found);
			// printk("xrp_scratch_page n_keys: %d\n",req->bio->xrp_scratch_page.n_keys);
			// printk("xrp_scratch_page bio addr: %p\n",&req->bio->xrp_scratch_page);
			if (!nvme_try_complete_req(req, cqe->status, cqe->result) &&
				!blk_mq_add_to_batch(req, iob, nvme_req(req)->status,
							nvme_pci_complete_batch))
