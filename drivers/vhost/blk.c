...
struct vhost_blk_req {
	unsigned long ib_bpf_ino;
	struct host_extent_status ib_es[15];
	unsigned int nr_ib_es;
			      ...
			      struct block_device *bdev)
{
	...
	if (pages_nr_total > NR_INLINE) {
		...
		req->bio = req->inline_bio;
		// printk("vhost_blk_bio_make: use inline bio.xrp_enable %d\n", req->bio[0]->xrp_enabled);
	}
		...
		for (j = 0; j < pages_nr; j++) {
     	unsigned int off, len, pos;
			...
			while (!bio || !bio_add_page(bio, page, len, off)) {
				...
				bio->xrp_count = 1;
				bio->xrp_inode = NULL;
				if(req->bi_opf == REQ_OP_READ && req->ib_bpf_ino > 0){ /*read op*/
					bio->xrp_enabled = 1;
					...
					if (bio->xrp_enabled) {
						//todo support two
						// bio->xrp_scratch_page._resume_from_leaf = 0;
						// bio->xrp_scratch_page.agg_op = 0;
						// bio->xrp_scratch_page.len = 0;
						// bio->xrp_scratch_page.flags = 0;
						// bio->xrp_scratch_page._state = 2;
						// bio->xrp_scratch_page._node_key_ix = 0;
						// bio->xrp_scratch_page.len = 0;
						// if (req->ib_bpf_ino == 2){
						// 	// begin, end, resume, 
						// 	bio->xrp_scratch_page.range_begin = req->ib_es[0].es_lblk;
						// 	bio->xrp_scratch_page.range_end = req->ib_es[1].es_lblk;
						// 	bio->xrp_scratch_page._resume_from_leaf = req->ib_es[2].es_lblk;
						// 	if (bio->xrp_scratch_page._resume_from_leaf != 0){
						// 		bio->xrp_scratch_page._state = 1;
						// 	}
						// 	req->nr_ib_es = 0;
						// 	// printk("vhost_blk_bio_make: begin keys %ld, end key %lu, _resume_from_leaf %u\n",
						// 	// 	 bio->xrp_scratch_page.range_begin, bio->xrp_scratch_page.range_end, bio->xrp_scratch_page._state);
						// }
						bio->xrp_scratch_page.n_keys = req->nr_ib_es;
						bio->xrp_scratch_page.current_index = 0;
						bio->xrp_scratch_page.state_flags = 0;
						bio->xrp_scratch_page.value_ptr = 0;
						bio->xrp_scratch_page.root_pointer = 0;
						for (m = 0; m < req->nr_ib_es; m++) {
							bio->xrp_scratch_page.keys[m] = req->ib_es[m].es_lblk;
						}
						// printk("vhost_blk_bio_make: bio_nr %d,n_keys %d, key %ld\n", bio_nr, req->nr_ib_es, bio->xrp_scratch_page.keys[0]);
						req->nr_ib_es = 0;
						if(!bdev->ib_enalbed || bdev->req_bpf_ino!= req->ib_bpf_ino){
							if (req->ib_bpf_ino == 1)
								bdev->xrp_bpf_prog = bpf_prog_get_type_path("/sys/fs/bpf/oliver_agg_func", BPF_PROG_TYPE_XRP);
							else
								bdev->xrp_bpf_prog = bpf_prog_get_type_path("/sys/fs/bpf/oliver_range_func", BPF_PROG_TYPE_XRP);
							bdev->req_bpf_ino = req->ib_bpf_ino;
							bdev->ib_enalbed = true;
							printk("vhost_blk_bio_make: bdev ib enable, bpf_ino %d\n", req->ib_bpf_ino);
						}
						if (IS_ERR(bdev->xrp_bpf_prog)) {
							printk("vhost_blk_bio_make: failed to get bpf prog\n");
							bdev->xrp_bpf_prog = NULL;
							bdev->ib_enalbed = false;
	...
	// printk("vhost_blk_bio_make: bio_nr %d\n",bio_nr);
...
static int vhost_blk_req_submit(struct vhost_blk_req *req, struct file *file)
{
	...
	int ret;
	int i;
	if (req->ib_bpf_ino > 0 && req->bi_opf == REQ_OP_WRITE) {
		if (req->nr_ib_es > 0) {
			for (i = 0; i < req->nr_ib_es; i++) {
				spin_lock(&inode->xrp_extent_lock);
				pr_info("The %dth es:lblk: %u; len: %u; pblk: %llu\n",i,req->ib_es[i].es_lblk,req->ib_es[i].es_len,req->ib_es[i].es_pblk);
...
static void vhost_blk_handle_host_kick(struct vhost_work *work)
{
	...
	while (llnode) {
		...
		vhost_blk_req_umap(req);
		if (req->ib_bpf_ino > 0 && req->bi_opf == REQ_OP_READ) {
			req->ibmsg.query.found = req->bio[0]->xrp_scratch_page.values[0].found;
			if (req->ibmsg.query.found == 1) {
				// pr_info("value is found\n");
				// printk("vhost_blk_handle_host_kick: value %s, bio addr %p", 
				// 	req->bio[0]->xrp_scratch_page.values[0].value, &req->bio[0]);
				memcpy(req->ibmsg.query.value, req->bio[0]->xrp_scratch_page.values[0].value, sizeof(val__t));
			} else {
				// pr_info("value is not found\n");
			}


			// req->ibmsg.query.found = req->bio[0]->xrp_scratch_page.flags;
			// for (i = 0; i < req->bio[0]->xrp_scratch_page.len; i++) {
			// 	printk("vhost_blk_handle_host_kick: %dth key %ld, value %s\n", i, 
			// 	req->bio[0]->xrp_scratch_page.kv[i].key,
			// 	req->bio[0]->xrp_scratch_page.kv[i].value);
			// }
			// printk("vhost_blk_handle_host_kick: key len %d\n", req->bio[0]->xrp_scratch_page.len);
		}
