...
#define XRP_RNG_KEYS 32
#define XRP_PTR_SIZE sizeof(unsigned long)
#define XRP_BLK_SIZE 512
#define BLK_SIZE_LOG 9
#define SCRATCH_SIZE 4096
#define XRP_META_SIZE sizeof(unsigned long)
#define XRP_KEY_SIZE sizeof(unsigned long)
#define XRP_NODE_CAPACITY ((XRP_BLK_SIZE - 2 * XRP_META_SIZE) / (XRP_KEY_SIZE + XRP_PTR_SIZE))
#define LOG_CAPACITY  ((XRP_BLK_SIZE) / (VAL_SIZE))

struct _Node {
    meta__t next;
    meta__t type;
    key__t key[XRP_NODE_CAPACITY];
    ptr__t ptr[XRP_NODE_CAPACITY];
};
...
struct RangeQuery {
    ...
    int len;
    struct KeyValue kv[XRP_RNG_KEYS];
    long agg_value;
